...
export function getLocalMonthBoundaries(
  ...
  const localDateString = formatInTimeZone(baseDate, timezone, "yyyy-MM-dd'T'HH:mm:ss.SSSXXX");
  ...
  return {
    ...
    // Mes Anterior
    prevMonthStart: toDate(startOfMonth(subMonths(localDate, 1)), { timeZone: timezone }),
    prevMonthEnd: toDate(endOfMonth(subMonths(localDate, 1)), { timeZone: timezone }),
    
    // Histórico (Ej: 3 años)
    historyStartDate: toDate(startOfMonth(subYears(localDate, 3)), { timeZone: timezone }),
  };
}